...
public class PictureLib {
	...
	File aoL = new File(d + "/res/0006L.png");    
	Image aoLi = null;
	File aoR = new File(d + "/res/0006R.png");    
	Image aoRi = null;
	File clL = new File(d + "/res/0106L.png");    
	Image clLi = null;
	File clR = new File(d + "/res/0106R.png");    
	Image clRi = null;
	
	public PictureLib(){
		try {
			aoRi = ImageIO.read(aoR);
		} catch (IOException e) {
			System.err.println("Could not find image" + d);
		}
		try {
			clRi = ImageIO.read(clR);
		} catch (IOException e) {
			System.err.println("Could not find image" + d);
		}
		try {
			aoLi = ImageIO.read(aoL);
		} catch (IOException e) {
			System.err.println("Could not find image" + d);
		}
		try {
			clLi = ImageIO.read(clL);
		} catch (IOException e) {
			System.err.println("Could not find image" + d);
		}
		try {
